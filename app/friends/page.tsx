...
function FriendDebtCard({ friend }: { friend: FriendDto }) {
  ...
  const mixedCurrencies = debt !== null && currency === null && (debt.bills.length ?? 0) > 0;

  return (
    <button
      onClick={() => router.push(ROUTES.friendDetail(friend.userId))}
      className="w-full text-left bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 hover:border-blue-300 dark:hover:border-blue-600 transition-colors"
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-slate-800 dark:text-slate-100 truncate">{friend.displayName}</p>
            {debt === null ? (
              ...
              <p className="text-xs text-slate-400">Multiple currencies</p>
            ) : (
